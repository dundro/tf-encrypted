...
class LogisticRegression:
    ...
    def fit(self, sess, x, y, num_batches):
        ...
        for batch in range(num_batches):
            ...
            sess.run(fit_batch_op, tag="fit-batch", write_trace=True)
...
class DataOwner:
    ...
    def __init__(
            self, player_name, num_features, training_set_size, test_set_size, batch_size, get_train_data=None,
            get_test_data=None
    ):
        ...
        self.get_train_data_fn = get_train_data
        self.get_test_data_fn = get_test_data
    ...
    def preprocess_data(self, train, has_labels):
        if train:
            dataset = self.get_train_data_fn()
        ...
        if has_labels:
            ...
            y = tf.reshape(y, [self.batch_size, 10])
